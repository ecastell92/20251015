...
def backup_s3_configurations() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Encontrados {len(bucket_arns)} buckets con tag {TAG_FILTER_KEY}={TAG_FILTER_VALUE}"
        )
        ...
        for arn in bucket_arns:
            ...
            try:
                ...
                bucket_config["encryption"] = encryption_response.get(
            ...
            # Inventory
            try:
                inventory_response = s3_client.list_bucket_inventory_configurations(
                    Bucket=bucket_name
                )
                bucket_config["inventory"] = inventory_response.get(
            ...
            try:
                ...
                bucket_config["replication"] = replication_response.get(
                    ...
                    glue_config["tables"].append(
                        {
                            ...
                            "storage_descriptor": {
                                ...
                                "serde_info": table.get("StorageDescriptor", {}).get(
                                    "SerdeInfo", {}
                                ),
                            },
                    ...
                    athena_config["workgroups"].append(
                        {
                            ...
                            "configuration": {
                                "result_configuration": wg_details.get("Configuration", {}).get(
                                    "ResultConfiguration", {}
                                ),
                                "enforce_work_group_configuration": wg_details.get(
                                    "Configuration", {}
                                ).get("EnforceWorkGroupConfiguration", False),
                                "publish_cloud_watch_metrics_enabled": wg_details.get(
                                    "Configuration", {}
                                ).get("PublishCloudWatchMetricsEnabled", False),
                                "bytes_scanned_cutoff_per_query": wg_details.get(
                                    "Configuration", {}
                                ).get("BytesScannedCutoffPerQuery"),
                                "requester_pays_enabled": wg_details.get("Configuration", {}).get(
                                    "RequesterPaysEnabled", False
                                ),
                                "engine_version": wg_details.get("Configuration", {}).get(
                                    "EngineVersion", {}
                                ),
                            },
                        }
                    ...
                    for stmt_summary in page.get("PreparedStatements", []):
                        ...
                        try:
                            stmt = athena_client.get_prepared_statement(
                                StatementName=stmt_name, WorkGroup=wg_name
                            )["PreparedStatement"]
    ...
    try:
        ...
        for page in paginator.paginate():
            for role in page["Roles"]:
                ...
                if not any(
                    keyword in role_name.lower()
                    for keyword in ["backup", "bck", "glue", "lambda", "data", "lake", "athena"]
                ):
                    continue
    ...
    try:
        ...
        for page in paginator.paginate():
            for rule in page["Rules"]:
                ...
                if not any(
                    keyword in rule_name.lower() for keyword in ["backup", "bck", "data", "sweep"]
                ):
                    continue
                ...
                tables_config.append(
                    {
                        ...
                        "billing_mode": table_desc.get("BillingModeSummary", {}).get(
                            "BillingMode", ""
                        ),
...
def backup_rds_configurations() -> Dict[str, Any]:
    ...
    rds_config = {"instances": [], "clusters": [], "parameter_groups": [], "subnet_groups": []}
